...
emissions_baseline = [pd.read_csv('data/yearly_CSV_agg_treated/datas'+str(year)+'/prod_CO2_WLD_with_agri_'+str(year)+'.csv').value.sum() for year in years]
...
intensities = pd.concat(
    [pd.read_csv('data/yearly_CSV_agg_treated/datas'+str(y)+'/co2_intensity_prod_with_agri_'+str(y)+'.csv').set_index(['country','sector']) for y in years],
    keys=years,names = ['year','country','sector']).reorder_levels([1,2,0]).sort_index()
output = pd.read_csv('data/yearly_CSV_agg_treated/datas2018/co2_intensity_prod_with_agri_2018.csv').set_index(['country','sector'])
...
country_intensities = pd.concat(
    [pd.read_csv('data/yearly_CSV_agg_treated/datas'+str(y)+'/prod_CO2_with_agri_'+str(y)+'.csv').set_index(['country','sector']).groupby(level=0).sum() \
     / \
...
#     [pd.read_csv('data/yearly_CSV_agg_treated/datas'+str(y)+'/prod_CO2_with_agri_'+str(y)+'.csv').set_index(['country','sector']).sum() \
#       / \
...
country_intensities = pd.concat(
    [pd.read_csv('data/yearly_CSV_agg_treated/datas'+str(y)+'/prod_CO2_with_agri_'+str(y)+'.csv').set_index(['country','sector']).xs(sector,level=1) \
     / \
