...
def compute_nodes_and_edges_baseline(traded, tot, sol_all, y, world=None):
...
def load_baseline_compute_initial(carb_cost,y,final_or_inter_or_total):
    ...
    if final_or_inter_or_total == 'final':
        for y in sol_all.keys():
            tot[y] = pd.concat({'cons':sol_all[y].cons}, names=['col_sector']).reorder_levels([1,2,3,0])
            temp = tot[y].reset_index()
            traded[y] = temp[temp['row_country'] != temp['col_country']].set_index(['row_country', 'row_sector', 'col_country','col_sector'])
            del temp
    ...
    nodes_unpivoted, edges_unpivoted, nodes_total, edges_total = \
        compute_nodes_and_edges_baseline(traded, tot, sol_all, y,world)
